    ...
    TreeDepth(0)
{
    ...
    for (int i = 0; i < Params.NumStartStates; i++)
      ...
      for(int j=0; j< NUM_PARTICLES; j++){  //numeri piccoli 
        Root->Bags().AddSample(Simulator.CreateStartState()); //corretto
    }
    Root->Bags().Display(std::cout, Simulator); //corretto
...
void MCTS::UCTSearch()
{
    ...
    for (int n = 0; n < Params.NumSimulations; n++)
    {
        ...
        //avrò due funzioni diverse di SimulateV e SimulateQ in quanto dovrò portarmi in giro le bag (e utilizzarle)
        //double totalReward = SimulateV_rho(*state, Root, Root->Bags());


        double totalReward = SimulateV_rho(*state, Root);
...
double MCTS::SimulateV_rho(STATE& state, VNODE* vnode)
{
    ...
    QNODE& qnode = vnode->Child(action);
    //std::cout << vnode->Bags().GetNumSamples() << std::endl;
      //  std::cout << "aggiungo un sample inutile" << std::endl;
      //  vnode->Bags().AddSample(&state, 1);
      //  vnode->Bags().Free(Simulator);
      //  std::cout << "già tolto, ma almeno ho inizializzato particle?" << std::endl;
    double totalReward = SimulateQ_rho(state, qnode, action, vnode->Bags());
    vnode->Value.Add(totalReward);
    AddRave(vnode, totalReward);
    return totalReward;
}

double MCTS::SimulateQ_rho(STATE& state, QNODE& qnode, int action, BAG& bag)
{
    int observation;
    double immediateReward, delayedReward = 0;
    //std::cout << bag.GetNumSamples() << std::endl;
...
//qua si dovrebbe generare la nuova bag di particle (BETA_t+1)
    //serve Bag al tempo t, modello Generativo del sistema
    //il peso di uno stato (weight associato) deve essere un numero (come nei rhopomcp in java) 
    //o è dato dalla distribuzione delle particle?

    //BAG& CreateBag(VNODE& bag_successiva, int action, int& observation, double& immediateReward, BAG& bag);
    ...
    if (!terminal)
    {
                //std::cout << "quante volte arrivi qua?" << std::endl;

        CreateBag(vnode ,action, observation,immediateReward, bag);
        //std::cout << "Bags del vnode dopo il createbag = " << std::endl;
        ...
        TreeDepth++;
        if (vnode){
            //std::cout << "primo ciclo simulateV-simulateQ-CreateBag fatto" << std::endl;
            delayedReward = SimulateV_rho(state, vnode);
        }
...
{
//std::cout << "copia venuta da mcts.addsample_bag" << std::endl;
...
void MCTS::CreateBag(VNODE*& bag_successiva, int action, int& observation, double& immediateReward, BAG& bag){
    ...
    //cout << "AddSample da CreateBag" << endl;
    bag_successiva->Bags().AddSample(s_particle,peso);
    //AddSample_Bag(bag_successiva,s_particle,peso);
}


/*
        STATE* bag_particle = bag.CreateSample(Simulator);
        STATE* state_particle = bag_particle;
        bool terminal = Simulator.Step(*state_particle, action, observation, immediateReward);
        double probability = Simulator.ProbObs(observation,*bag_particle,action, *state_particle);

        int peso = probability * NUM_PARTICLES;
        AddSample_Bag(bag_successiva,*state_particle,peso);


    //campiono uno stato s'dalla bag
    double counter =0;
    for(int i = 0; i< bag.GetNumSamples(); ++i){
        counter +=bag.GetWeight(i);
    }
    newBag.AddSample(state_particle,counter/bag.GetNumSamples());
